...
function mphb_tmpl_the_room_type_galery( $atts = array() ){
	...
	$result .= '</div>';
	$result .= '<script>jQuery(".mphb-room-type-gallery-wrapper").detach().prependTo(".entry-content")</script>';
...
function mphb_tmpl_the_room_type_calendar( $roomType = null, $atts = '' ) {
	...
	// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
	<div class="mphb-calendar mphb-datepick inlinePicker" id="mphb-calendar-<?php echo esc_attr( $roomType->getId() ); ?>"<?php echo $atts; ?>></div>
	<?php
}
...
function mphb_tmpl_the_room_reservation_form( $roomType = null ){
	...
	<form method="<?php echo esc_attr( $formMethod ); ?>" action="<?php echo esc_url( $actionUrl ); ?>" class="mphb-booking-form" id="<?php echo esc_attr( 'booking-form-' . $typeId ); ?>">
		...
		<p class="mphb-reserve-btn-wrapper">
			<input class="mphb-reserve-btn button" disabled="disabled" type="submit" value="<?php esc_attr_e( 'Check Availability', 'motopress-hotel-booking' ); ?>" />
        ...
        <tbody>
            ...
            <?php } else { ?>
                ...
                foreach ($payments as $payment) {
                    ...
                    echo '<td>',date_i18n( 'M j Y', $payment->getDate()->format( 'U' ) ), '</td>';
                    echo '</tr>';
                }
                ?>
            <?php } ?>
